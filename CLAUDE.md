...
This repo is a Claude Code plugin. It extends `claude-code-hermit` (core v0.2.4+) with software development capabilities.
...
- `claude-code-hermit` v0.2.4+ (core) — session discipline, task tracking, cost tracking, OPERATOR.md, learning loop

## Core Contracts

1. Profile-gating via `AGENT_HOOK_PROFILE` env var (`minimal`/`standard`/`strict`)
2. Session lifecycle: dev workflow operates within core's session loop; `/session-close` is never called at task boundaries — only by the operator
3. State dir: `.claude-code-hermit/` (sessions/, proposals/, templates/)
4. Learning loop: dev workflow invokes `reflect` at every task boundary (supplementary to core's independent triggers)
